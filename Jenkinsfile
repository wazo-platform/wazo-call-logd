  ...
  stages {
    stage('Debian build and deploy') {
      steps {
        build job: 'build-package-no-arch', parameters: [
          string(name: 'PACKAGE', value: "wazo-call-logd"),
    ...
    stage('Docker build') {
      steps {
        ...
        sh "docker build --no-cache -t wazoplatform/wazo-call-logd:bookworm ."
      }
    }
    stage('Docker publish') {
      steps {
        sh "docker push wazoplatform/wazo-call-logd:bookworm"
      }
    }
  }
  post {
